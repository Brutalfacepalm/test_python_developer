...
class GridCity:
    def __init__(self, N, M, R, observ_p, density=6):
        self.N = N
        self.M = M
        self.R = R
        ...
        self.V = self.N * self.M // 2
        self.S = 1

        _border = int(self.N * self.M * self.observ_p)
        self.city = np.concatenate([np.ones((_border,)) * self.N * self.M, np.zeros((self.N * self.M - _border,))])
        self.coverage = np.zeros((self.N, self.M))
        np.random.shuffle(self.city)
        self.city = np.reshape(self.city, (self.N, self.M))
        self.influence = [[[] for _ in range(self.M)] for _ in range(self.N)]
    ...
    def activate_tower(self, x, y, tower):
        ...
        for y_i in range(max(0, y - tower.range), min(y + tower.range + 1, self.N)):
            for x_i in range(max(0, x - tower.range), min(x + tower.range + 1, self.M)):
    ...
    def _find_start(self):
        ...
        while x_start or y_start:
            if self.city[y_start, x_start] != self.N * self.M:
                break
            if x_start < y_start:
    ...
    def _find_free_place(self, x, y, down, left, slice_down=False):
        if slice_down:
            while y - self.R >= 0 and 0 in self.coverage[max(0, y - self.R),
                                           max(0, x - self.R): min(x + self.R + 1, self.M - 1)]:
                y -= 1
            while y + self.R < self.N and 0 not in self.coverage[min(y - self.R + 1, self.N - 1),
                                                   max(0, x - self.R): min(x + self.R + 1, self.M - 1)]:
                y += 1
        if self.coverage[y, x] < self.density:
            if self.city[y, x] == self.N * self.M:

                while self.city[y, x] == self.N * self.M and (x >= 0 and y >= 0) and (x < self.M and y < self.N):
                    if down:
                        y -= 1
                        if self.city[y, x] == 0:
                            break
            ...
            if self.budget and (self.cost_tower or self.towers_available):
                if self.towers_available:
                    ...
                    for cost_tower, range_tower in self.towers_available:
                        if cost_tower <= self.budget:
                            cost_per_cell = np.count_nonzero(
                                self.coverage[max(0, y - range_tower): min(y + range_tower + 1, self.N),
                                max(0, x - range_tower): min(x + range_tower + 1, self.M)] == 0) / cost_tower
                            if cost_per_cell >= max_cost_per_cell:
                    ...
                    additional_coverage = np.count_nonzero(
                        self.coverage[max(0, y - self.R): min(y + self.R + 1, self.N),
                        max(0, x - self.R): min(x + self.R + 1, self.M)] == 0)
                    return x, y, additional_coverage, self.R, self.cost_tower
            else:
                return x, y, 0, self.R, 0
    ...
    def replacement_towers(self, optimize_hops=False):
        ...
        while border_coverage:
            ...
            if self.budget and (self.cost_tower or self.towers_available):
                if cost_tower > self.budget:
                    continue
            if np.min(self.coverage[max(0, y - range_tower): min(y + range_tower + 1, self.N),
                      max(0, x - range_tower): min(x + range_tower + 1, self.M)]) < self.density and self.city[
                y, x] == 0:
                tower = Tower(range_tower, self.V, self.S, x, y)
            ...
            if optimize_hops:
                next_tower = self._find_free_place(min(max(range_tower // 2, x + range_tower), self.M - 1),
                                                   min(max(range_tower // 2, y + range_tower), self.N - 1), down=True,
                                                   left=True)
                if next_tower:
                    border_coverage.append(next_tower)
                next_tower = self._find_free_place(min(max(range_tower // 2, x - range_tower), self.M - 1),
                                                   min(max(range_tower // 2, y - range_tower), self.N - 1), down=False,
                                                   left=False)
                if next_tower:
                    border_coverage.append(next_tower)
                next_tower = self._find_free_place(min(max(range_tower // 2, x + range_tower), self.M - 1),
                ...
                next_tower = self._find_free_place(min(max(range_tower, x + range_tower * 2 - 2), self.M - 1),
                                                   min(max(range_tower, y + 1), self.N - 1), down=True, left=True,
                                                   slice_down=True)
                if next_tower:
                    border_coverage.append(next_tower)
                if x + range_tower >= self.M:
                    x = range_tower
                    next_tower = self._find_free_place(min(max(range_tower, x), self.M - 1),
                                                       min(max(range_tower, y + range_tower - 1), self.N - 1),
                                                       down=True, left=True, slice_down=True)
                    if next_tower:
                        border_coverage.append(next_tower)

            if self.budget and (self.cost_tower or self.towers_available):
                border_coverage = sorted(border_coverage, key=lambda x: x[2])

    def build_network(self):
        for x, y in self.towers_coordinates:
            for main_tower in self.influence[y][x]:
                for tower_neighborhood in self.influence[y][x]:
                    if main_tower is not tower_neighborhood and tower_neighborhood not in main_tower.neighborhood and tower_neighborhood in \
                            self.influence[main_tower.y][main_tower.x]:
    ...
    def visualize(self, path=False, coverage=False, save_image=''):
        data = []
        if coverage:
    ...
    def run_experiment(self, task_number, image_path=''):
        ...
        if task_number == 2:
            ...
            self.visualize(path=path_between_towers, coverage=True, save_image=image_path)
        if task_number == 3:
            self.set_budget_and_cost_tower(np.random.randint(1000, 30000), np.random.randint(10, 100))
            self.replacement_towers(optimize_hops=True)
            self.build_network()
